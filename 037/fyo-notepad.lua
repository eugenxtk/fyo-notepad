...
if not doesFileExist(CONFIG_FILE_PATH) then
	...
	for idx = 1, MAX_NOTES do
		...
		defaultNotesConfig[idx]['name'] = 'Íàçâàíèå çàìåòêè'

		defaultNotesConfig[idx]['text'] = 'Ââåäèòå òåêñò çàìåòêè'
...
function main()
	...
	sampRegisterChatCommand('note', openGivenNote)
...
function imgui.OnDrawFrame()
	...
	if menuState.v then
		...
		imgui.Begin(u8('Ìåíþ'), menuState, imgui.WindowFlags.AlwaysAutoResize)

		for idx = 1, MAX_NOTES do
			useCustomInputText(string.format('##%d', idx), noteNames[idx], string.format('%d', idx))
			imgui.Text('  ')
			imgui.SameLine()
			if imgui.Button(u8(string.format('%s##%d', not noteStates[idx].v and 'Îòêðûòü' or 'Çàêðûòü', idx))) then
				noteStates[idx].v = not noteStates[idx].v
			end
		...
		imgui.Separator()
		useCustomInputText('##maxLineLength', maxLineLength, u8('Ìàêñ. äëèíà ñòðîêè:'))
